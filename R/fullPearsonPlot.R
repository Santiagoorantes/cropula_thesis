...
fullPearsonPlot <- function(data, boot = NULL, method = "unbiased", alpha = 0.8,
  ...
  df_r6 <- data.frame("x" = c(25,3+21,3,3*(a+5)*(a-2)/(a-3)/(a-4)),
  ...
  lnorm_df <- data.frame("x" = c(3, (es2^4+2*es2^3+3*es2^2-3)),
                         "y" = c(0, (es2+2)^2*(es2-1)))
  p <- p + geom_line(data = lnorm_df,
                     aes(x = x, y = y, colour = "Lognormal"),
                     linetype = 2, size = 1.2, colour = line_col[2])
  
  # Inverse Gamma (V)
  invgam_df <- data.frame("x" = c(3, 3*(a+5)*(a-2)/(a-3)/(a-4)),
                          "y" = c(0, 16*(a-2)/(a-3)^2))
  p <- p + geom_line(data = invgam_df,
                     aes(x = x, y = y, colour = "Lognormal"),
  ...
  df_r4 <- data.frame("x" = c(3*(a+5)*(a-2)/(a-3)/(a-4),3,25),
  ...
  if (!is.null(boot)) {
    ...
    p <- p + geom_point(data = boot_df, aes(x, y),
  ...
  # Annotations
  p <- p + 
    annotate("text", x = 4, y = 10, label = "Imposible",
             family = "serif", fontface = "bold", colour = annotation_col,
             size = text_size) +
    annotate("text", x = 12, y = 9, label = "Beta (I)",
             family = "serif", fontface = "bold", colour = annotation_col,
             size = text_size, angle = 0) +
    annotate("text", x = 15, y = 8.3, label = "Gamma (III)",
             family = "serif", fontface = "bold", colour = annotation_col,
             size = text_size, angle = 29) +
    annotate("text", x = 20, y = 9, label = "Beta prime or F (VI)",
             family = "serif", fontface = "bold", colour = annotation_col,
             size = text_size, angle = 0) +
    annotate("text", x = 16, y = 6.6, label = "Lognormal",
             family = "serif", fontface = "bold", colour = annotation_col,
             size = text_size, angle = 20) +
    annotate("text", x = 18, y = 6.1, label = "Inverse gamma (V)",
             family = "serif", fontface = "bold", colour = annotation_col,
             size = text_size, angle = 15) +
    annotate("text", x = 20, y = 3, label = "(IV)",
             family = "serif", fontface = "bold", colour = annotation_col,
             size = text_size, angle = 0) +
    annotate("text", x = 15, y = 0, label = "t (VII)",
             family = "serif", fontface = "bold", colour = annotation_col,
             size = text_size, angle = 0)
